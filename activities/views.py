...
class ActivityCategoryListView(generics.ListAPIView):
    ...
    @extend_schema(
        summary="Faoliyat kategoriyalari ro'yxati",
        description="Barcha faol faoliyat kategoriyalarini qaytaradi.",
        responses={200: ActivityCategoryListSerializer(many=True)},
    )
...
@extend_schema(tags=['Activities'])
class ActivityCategoryDetailView(generics.RetrieveAPIView):
    """
    ...
    @extend_schema(
        summary="Faoliyat kategoriyasi tafsilotlari",
        description="Bitta kategoriyaning to'liq ma'lumotlarini va uning sahifalarini qaytaradi.",
        responses={200: ActivityCategoryDetailSerializer},
    )
    def get(self, request, *args, **kwargs):
        return super().get(request, *args, **kwargs)
...
@extend_schema(tags=['Activities'])
class ActivityPageDetailView(generics.RetrieveAPIView):
    """
